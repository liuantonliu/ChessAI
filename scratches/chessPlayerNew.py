...
def GetB(r,pos):
    ...
    mycol = GetCol(r,pos)
    temppos = pos
    while (temppos)%8 <= 6 and OnBoard(pos+9):
        ...
        if GetCol(r,temppos) != 0:
            break
    temppos = pos
    while (temppos)%8 <= 6 and OnBoard(pos-7):
        ...
        if GetCol(r,temppos) != 0:
            break
    temppos = pos
    while (temppos)%8 >= 1 and OnBoard(pos+7):
        if GetCol(r, temppos+7) != mycol:
            temppos += 7
            out += [temppos]
        if GetCol(r, temppos+7) == mycol:
        ...
        if GetCol(r, temppos) != 0:
            break
    temppos = pos
    while (temppos) % 8 >= 1 and OnBoard(pos-9):
        if GetCol(r, temppos-9) != mycol:
            temppos -= 9
            out += [temppos]
        if GetCol(r, temppos-9) == mycol:
...
def EvalMoves(r,moves,player):
    out1 = []
    for move in moves:
        if IsPositionUnderThreat(r,move,player) == False:
            out1 += [move]
    out2 = []
...
# changepos = 58
# board[changepos] = 21
# print(GetPiecesLegalMoves(board,changepos))
# printBoard(board)
